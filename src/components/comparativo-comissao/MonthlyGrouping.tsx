  ...
  return (
    <TableRow className={zebra ? "bg-muted/25" : undefined}>
      ...
      <TableCell className="text-right tabular-nums">
        {formatPercent(g.percentualMedioPonderado)}
      </TableCell>
      <TableCell className="text-right tabular-nums">{formatMoney(g.vgvEquivalente6)}</TableCell>
      <TableCell
        className={`text-right tabular-nums ${g.diferenca > 0 ? "text-destructive" : g.diferenca < 0 ? "text-blue-700 dark:text-blue-400" : ""}`}
      >
...
function GrupoTable({
  ...
  rows: { label: string; g: GrupoMensal | GrupoSecundario }[];
  labelHeader: string;
}) {
  return (
          ...
          {rows.length === 0 && (
            <TableRow>
              <TableCell colSpan={7} className="py-6 text-center text-sm text-muted-foreground">
                Sem dados no período/filtro selecionado.
              </TableCell>
            </TableRow>
          )}
          {rows.map(({ label, g }, i) => (
            <GrupoRow key={label} label={label} g={g} zebra={i % 2 === 1} />
          ))}
        </TableBody>
      </Table>
    </div>
...
export function MonthlyGrouping({ rows }: { rows: ComparativoRowComCalculo[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Agrupamento adicional</CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="corretor">
            ...
            <TabsContent value="corretor">
              <GrupoTable
                labelHeader="Corretor"
                rows={porCorretor.map((g) => ({ label: g.label, g }))}
              />
            </TabsContent>
            <TabsContent value="equipe">
              <GrupoTable
                labelHeader="Equipe"
                rows={porEquipe.map((g) => ({ label: g.label, g }))}
              />
            </TabsContent>
            <TabsContent value="gestor">
              <GrupoTable
                labelHeader="Gestor/Team leader"
                rows={porGestor.map((g) => ({ label: g.label, g }))}
              />
            </TabsContent>
          </Tabs>
        </CardContent>
