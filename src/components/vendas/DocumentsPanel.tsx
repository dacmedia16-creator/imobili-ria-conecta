...
export type DisplayDocument = Omit<
  DocumentRow,
  "file_name" | "parte" | "status" | "storage_path" | "tipo"
> & {
  file_name: string;
  parte: DocParte;
...
export function DocumentsPanel({
  ...
  const [preview, setPreview] = useState<{ doc: DisplayDocument; url: string } | null>(null);
  const [zoom, setZoom] = useState(1);
  const [printingAll, setPrintingAll] = useState(false);
  const [downloadingAll, setDownloadingAll] = useState(false);
  const [pendingReject, setPendingReject] = useState<DisplayDocument | null>(null);
  const [rejectMotivo, setRejectMotivo] = useState("");
  const [rejecting, setRejecting] = useState(false);
  // Certidões pedidas pelo jurídico: linhas dinâmicas (nome + upload) — "+" adiciona mais uma.
  const [certidaoDrafts, setCertidaoDrafts] = useState<{ id: string; nome: string }[]>([
  ...
  const uploadCertidao = async (draftId: string, nome: string, file: File) => {
    ...
    try {
      ...
      await supabase.from("activity_logs").insert({
        ...
        payload: { tipo: "certidao_juridico", descricao: nome },
      });
  ...
  const viewDoc = async (doc: DisplayDocument) => {
  ...
  const printDoc = async (doc: DisplayDocument) => {
    const { data, error } = await supabase.storage
      .from("sale-documents")
      .createSignedUrl(doc.storage_path, 300);
    if (error || !data) {
  ...
  const printAllDocs = async () => {
    ...
    setPrintingAll(true);
    try {
      const { data, error } = await supabase.storage.from("sale-documents").createSignedUrls(
  ...
  const downloadAllAsPdf = async () => {
    ...
    try {
      ...
      toast.error(errorMessage(err, "Falha ao gerar PDF"));
    } finally {
      setDownloadingAll(false);
    }
  };

  const removeDoc = async (doc: DisplayDocument) => {
    setDeleting(true);
    try {
      ...
      await supabase.from("activity_logs").insert({
        sale_id: saleId,
        autor_id: user!.id,
        acao: "document_archived",
        payload: { doc_id: doc.id, tipo: doc.tipo, parte: doc.parte, file_name: doc.file_name },
      });
  ...
  const runExtraction = useCallback(
    async (documentId: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          toast.error(`Falha ao ler documento: ${res.error}`);
          return;
        }
        ...
        toast.success(
          applied.filled.length
            ? `Documento lido pela IA • ${applied.filled.length} campo(s) preenchido(s)`
            : "Documento lido pela IA",
        );
      } catch (err: unknown) {
        toast.error(errorMessage(err, "Falha ao extrair dados"));
      } finally {
  ...
  const copyFromBase = async (tipo: string, parte: DocParte) => {
    ...
    const baseDocs = docs.filter(
      (d) => d.tipo === tipo && (d.parte ?? "outros") === baseParte && d.status !== "recusado",
    );
    const baseDoc = baseDocs[baseDocs.length - 1];
    if (!baseDoc) {
      toast.error(`Envie primeiro o documento de ${parteLabel(baseParte)}`);
      return;
    }
    // RPC em vez de insert direto: mesmo bug de RLS inconsistente do PostgREST já corrigido
    // pra archive_sale_document (ver comentário em removeDoc) também acontecia aqui.
    const { error } = await supabase.rpc("insert_sale_document", {
      ...
      _tipo: tipo,
      _parte: parte,
      _storage_path: baseDoc.storage_path,
      _file_name: baseDoc.file_name,
      _status: baseDoc.status,
      _extraction_status: "done",
    });
    if (error) {
      toast.error(error.message);
      return;
    ...
    const campo =
      tipo === "comprovante_endereco"
        ...
        : tipo === "certidao"
          ...
          : null;
    if (campo) {
      const { data: baseParty } = await supabase
        .from("sale_parties")
        .select("endereco, regime_casamento")
        .eq("sale_id", saleId)
        .eq("papel", baseParte)
        .maybeSingle();
      const valor = baseParty?.[campo];
      if (valor) {
        const { data: existingParty } = await supabase
          .from("sale_parties")
          .select("id, endereco, regime_casamento")
          .eq("sale_id", saleId)
          .eq("papel", parte)
          .maybeSingle();
        if (existingParty) {
          if (!existingParty[campo]) {
            const patch = campo === "endereco" ? { endereco: valor } : { regime_casamento: valor };
            await supabase.from("sale_parties").update(patch).eq("id", existingParty.id);
          }
        } else {
          if (campo === "endereco") {
    ...
    await supabase.from("activity_logs").insert({
      sale_id: saleId,
      autor_id: user!.id,
      acao: "document_reused_from_other_party",
      payload: { tipo, parte, de: baseParte },
    });
    toast.success(`Documento reaproveitado de ${parteLabel(baseParte)}`);
    onChange();
  };

  const approve = async (doc: DisplayDocument) => {
    ...
    await supabase.from("activity_logs").insert({
      sale_id: saleId,
      autor_id: user!.id,
      acao: "document_approved",
      payload: { doc_id: doc.id, tipo: doc.tipo },
    });
    onChange();
  };
  const openRejectDialog = (doc: DisplayDocument) => {
    setRejectMotivo("");
    setPendingReject(doc);
  };
  const reject = async () => {
    ...
    try {
      ...
      await supabase.from("sale_comments").insert({
        ...
        autor_id: user!.id,
        escopo: "revisao",
        texto: `Documento recusado: ${motivo}`,
        doc_id: doc.id,
      });
      await supabase.from("activity_logs").insert({
        sale_id: saleId,
  ...
  const applyAll = async () => {
    ...
    try {
      ...
      const pendentes = docs.filter(
        (d) =>
          d.extraction_status !== "done" && d.tipo !== "contrato" && d.tipo !== "contrato_assinado",
      );
      let lidos = 0;
      let falhas = 0;
      ...
      toast.error(errorMessage(err, "Falha ao aplicar dados"));
    } finally {
      setApplying(false);
      setProgress(null);
    }
  ...
  const [compradorExtras, setCompradorExtras] = useState<number[]>(() =>
    numerosExtras("comprador"),
  );
  const [vendedorExtras, setVendedorExtras] = useState<number[]>(() => numerosExtras("vendedor"));
  useEffect(() => {
    setCompradorExtras((prev) =>
      Array.from(new Set([...prev, ...numerosExtras("comprador")])).sort((a, b) => a - b),
    );
    setVendedorExtras((prev) =>
      Array.from(new Set([...prev, ...numerosExtras("vendedor")])).sort((a, b) => a - b),
    );
  ...
  const tiposPessoalDaParte = (parte: string) =>
    docTypesPessoalPara(parties[parte]?.tipo_pessoa === "juridica" ? "juridica" : "fisica");
  const blocos: { parte: DocParte; tipos: typeof DOC_TYPES }[] = [
    { parte: "comprador_1", tipos: tiposPessoalDaParte("comprador_1") },
    ...compradorExtras.map((n) => ({
      parte: `comprador_${n}` as DocParte,
      tipos: tiposPessoalDaParte(`comprador_${n}`),
    })),
    { parte: "vendedor_1", tipos: tiposPessoalDaParte("vendedor_1") },
    ...vendedorExtras.map((n) => ({
      parte: `vendedor_${n}` as DocParte,
      tipos: tiposPessoalDaParte(`vendedor_${n}`),
    })),
  ...
  useEffect(() => {
    ...
    const pendentes = docs.filter(
      (d) =>
        ...
        d.tipo !== "contrato" &&
        d.tipo !== "contrato_assinado" &&
        d.parte !== "juridico" &&
        !autoQueuedIdsRef.current.has(d.id),
    );
    if (pendentes.length > 0) {
      for (const d of pendentes) autoQueuedIdsRef.current.add(d.id);
      autoJobQueueRef.current.push({
        parte: pendentes[0].parte,
        ids: pendentes.map((d) => d.id),
      });
    ...
    <div className="space-y-6 print:hidden">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <p className="text-muted-foreground">
                  Assim que você envia um documento, a IA já lê sozinha (em alguns segundos) e
                  roteia os dados para a pessoa certa nas próximas etapas — sem precisar clicar em
                  nada.
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {canUseAi && (
              <Button
                size="sm"
                onClick={applyAll}
                disabled={docs.length === 0 || applying || !editable}
              >
                {applying ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ...
                  <>
                    ...
                    Ler documentos e aplicar dados
                  </>
                )}
              </Button>
            )}
      ...
      <Wizard
        steps={enabledBlocos.map(({ parte, tipos }) => {
          ...
          const parteAccent = parte.startsWith("comprador_")
            ? "border-l-4 border-l-blue-500"
            : parte.startsWith("vendedor_")
              ...
              <section className="space-y-3">
                ...
                {tipos.map((t) => {
                  const list = docs.filter(
                    (d) => d.tipo === t.key && (d.parte ?? "outros") === parte,
                  );
                  ...
                  const dispensadoPorCnh =
                    (t.key === "rg" || t.key === "cpf") && temDocDoTipo(docs, "cnh", parte);
                  const cnhDispensadaPorRgCpf =
                    t.key === "cnh" &&
                    temDocDoTipo(docs, "rg", parte) &&
                    temDocDoTipo(docs, "cpf", parte);
                  const obrigatorioEfetivo =
                    t.key === "cnh" ? !cnhDispensadaPorRgCpf : t.obrigatorio && !dispensadoPorCnh;
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <div className="text-sm font-medium">
                              {t.label}
                              {obrigatorioEfetivo && mostraObrigatorio ? (
                                <span className="ml-1 text-destructive">*</span>
                              ) : null}
                            </div>
                            {t.key === "outros" && (
                              <div className="text-xs text-muted-foreground">
                                Ex.: procuração, declaração de união estável, comprovante de renda,
                                distrato, laudêmio, autorização de venda
                              </div>
                            )}
                            {obrigatorioEfetivo && mostraObrigatorio && (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {editable &&
                              list.length === 0 &&
                              parteBase(parte) &&
                              REUSABLE_DOC_TYPES.has(t.key) && (
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  onClick={() => copyFromBase(t.key, parte)}
                                >
                            ...
                            {podeEnviarAqui && (
                              <label className="inline-flex cursor-pointer items-center gap-1 rounded-md border px-3 py-1.5 text-sm hover:bg-muted">
                                ...
                                <input
                                  ...
                                  className="hidden"
                                  onChange={(e) =>
                                    e.target.files?.[0] && upload(t.key, parte, e.target.files[0])
                                  }
                                />
                              </label>
                            )}
                              ...
                              {canUseAi &&
                                ...
                                !extracting[d.id] && (
                                  <Button
                                    size="sm"
                                    variant="ghost"
                                    title="Ler novamente com IA"
                                    onClick={() => runExtraction(d.id)}
                                  >
                ...
                {parte === "juridico" && (
                  <>
                    {docs
                      .filter((d) => d.tipo === "certidao_juridico")
                      .map((d) => (
                          ...
                          <CardContent className="flex flex-wrap items-center gap-2 p-4">
                            ...
                            <label className="inline-flex cursor-pointer items-center gap-1 rounded-md border px-3 py-1.5 text-sm hover:bg-muted">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  e.target.files?.[0] &&
                                  uploadCertidao(draft.id, draft.nome, e.target.files[0])
                                }
                              />
                            </label>
                          </CardContent>
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex gap-2">
                    {editable && parte.startsWith("comprador_") && (
                      <Button size="sm" variant="outline" onClick={() => addParte("comprador")}>
                        + Adicionar comprador
                      </Button>
                    )}
                    {editable && parte.startsWith("vendedor_") && (
                      <Button size="sm" variant="outline" onClick={() => addParte("vendedor")}>
                        + Adicionar vendedor/proprietário
                      </Button>
                    )}
                  </div>
                  <div className="ml-auto flex items-center gap-2">
                    ...
                    {enabledBlocos.findIndex((b) => b.parte === parte) <
                      enabledBlocos.length - 1 && (
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(o) => !o && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingDelete?.file_name} sairá da lista. Depois disso você pode enviar um novo
              arquivo — a IA fará a leitura novamente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog
        open={!!pendingReject}
        onOpenChange={(o) => {
          if (!rejecting && !o) setPendingReject(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Descreva o motivo da recusa. O corretor será notificado.
            </DialogDescription>
          </DialogHeader>
          <Textarea
          ...
          />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setPendingReject(null)} disabled={rejecting}>
              Cancelar
            </Button>
            <Button onClick={reject} disabled={rejecting || !rejectMotivo.trim()}>
              Recusar
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-w-3xl">
            ...
            <div className="max-h-[70vh] overflow-auto rounded-md border bg-muted/30">
              {isImageFile(preview.doc.file_name) ? (
                <img
                  ...
                  style={
                    zoom === 1
                      ? { maxHeight: "70vh", maxWidth: "100%", width: "auto" }
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                preview &&
                printDocumentUrls([{ file_name: preview.doc.file_name, url: preview.url }])
              }
            >
              <Printer className="mr-2 h-4 w-4" />
              Imprimir
