...
export function PartiesStep({
  ...
  const [forms, setForms] = useState<Record<string, PartyForm>>(() => {
    const m: Record<string, PartyForm> = {};
    papeis.forEach((p) => {
      m[p] = parties[p] ?? {};
    });
    return m;
  ...
  const copiarContaDe = (papel: string, deOutroPapel: string) => {
    ...
    setBankForms((f) => ({
      ...
      [papel]: {
        ...f[papel],
        banco: origem.banco ?? null,
        agencia: origem.agencia ?? null,
        conta: origem.conta ?? null,
        pix: origem.pix ?? null,
      },
    }));
    setBankDirty((d) => ({ ...d, [papel]: true }));
  ...
  useEffect(() => {
    for (const [papel, party] of Object.entries(parties)) {
      if (party?.cliente_id && clienteMatch[papel] === undefined) {
        supabase
          .rpc("cliente_historico", { _cliente_id: party.cliente_id, _excluir_sale_id: saleId })
          .then(({ data: historico }) => {
  ...
  const [clienteMatch, setClienteMatch] = useState<
    Record<
      string,
      {
        historico: {
  ...
  const buscarCliente = async (papel: string, cpfDigitado: string | null | undefined) => {
    ...
    if (normalizado.length < 11) {
      setClienteMatch((m) => ({ ...m, [papel]: null }));
      return;
    }
    ...
    setDirty((d) => ({ ...d, [papel]: true }));
    const { data: historico } = await supabase.rpc("cliente_historico", {
      _cliente_id: cliente.id,
      _excluir_sale_id: saleId,
    });
    setClienteMatch((m) => ({ ...m, [papel]: { historico: historico ?? [] } }));
  };
    ...
    try {
      for (const papel of papeis) {
        ...
        const clientePayload = {
          ...
          nome: forms[papel].nome ?? null,
          razao_social: forms[papel].razao_social ?? null,
          cnpj: forms[papel].cnpj ?? null,
          cpf_cnpj: forms[papel].cpf_cnpj ?? null,
          rg: forms[papel].rg ?? null,
          profissao: forms[papel].profissao ?? null,
          email: forms[papel].email ?? null,
        ...
        if (normalizado.length >= 11) {
          ...
          if (clienteId) {
            const { error: clienteError } = await supabase
              .from("clientes")
              .update({ ...clientePayload, updated_by: user?.id ?? null })
              .eq("id", clienteId);
            if (clienteError) {
            ...
            const { data: novoCliente, error: clienteError } = await supabase
              .from("clientes")
              .insert({ ...clientePayload, created_by: user?.id ?? null })
              .select("id")
              .single();
            if (clienteError) {
              ...
              if (clienteError.code === "23505") {
                const { data: achouDeNovo } = await supabase
                  .from("clientes")
                  .select("id")
                  .eq("cpf_cnpj_normalizado", normalizado)
                  .maybeSingle();
      ...
      for (const papel of papeis) {
        ...
        const { error } = existingBank
          ...
          : await supabase
              .from("sale_bank_accounts")
              .insert({ sale_id: saleId, parte: papel, ...bankData });
        if (error) {
          toast.error(error.message);
          return false;
        }
    ...
    <div className="space-y-4">
      ...
      <Wizard
        steps={papeis.map((p, i) => {
          ...
          const outrosVendedoresComConta = papeis.filter(
            (op) =>
              op.startsWith("vendedor_") &&
              op !== p &&
              (bankForms[op]?.banco || bankForms[op]?.conta || bankForms[op]?.pix),
          );
          return {
            key: p,
            label: parteLabel(p),
              ...
              <Card>
                ...
                <CardContent>
                  <FieldGrid>
                    <Field label="Tipo de pessoa">
                      <Select
                        value={forms[p].tipo_pessoa ?? "fisica"}
                        onValueChange={(v) => update(p, "tipo_pessoa", v)}
                        disabled={!editable}
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  continua sendo uma pessoa física, então ainda precisa de RG/CPF/Profissão dela. */}
                    {(forms[p].tipo_pessoa ?? "fisica") === "juridica" && (
                      <>
                        <Field label="Razão social">
                          <Input
                            value={forms[p].razao_social ?? ""}
                            onChange={(e) => update(p, "razao_social", e.target.value)}
                            disabled={!editable}
                          />
                        </Field>
                        <Field label="CNPJ">
                          <Input
                            value={forms[p].cnpj ?? ""}
                            onChange={(e) => update(p, "cnpj", e.target.value)}
                            disabled={!editable}
                          />
                        </Field>
                      </>
                    ...
                    <Field label="RG">
                      <Input
                        value={forms[p].rg ?? ""}
                        onChange={(e) => update(p, "rg", e.target.value)}
                        disabled={!editable}
                      />
                    </Field>
                    <Field label="CPF">
                          ...
                          {clienteMatch[p]!.historico.length > 0 ? (
                            <div className="mt-1.5 space-y-1">
                              {clienteMatch[p]!.historico.map((h) => (
                                <div
                                  key={h.sale_id}
                                  className="flex items-center justify-between gap-2 rounded border border-emerald-200 bg-background px-2 py-1 text-xs dark:border-emerald-900"
                                >
                                  <span className="font-medium">
                                    {h.papel.startsWith("comprador") ? "Comprou" : "Vendeu"}
                                  </span>
                                  <span className="min-w-0 flex-1 truncate text-muted-foreground">
                                    {h.imovel_endereco ||
                                      h.imovel_id ||
                                      "Imóvel sem endereço cadastrado"}
                                  </span>
                            ...
                            <div className="mt-1 text-xs text-muted-foreground">
                              Primeira vez que esse CPF aparece numa venda. Edite os campos abaixo
                              se algo mudou.
                            </div>
                          )}
                        </div>
                    ...
                    <Field label="Profissão">
                      <Input
                        value={forms[p].profissao ?? ""}
                        onChange={(e) => update(p, "profissao", e.target.value)}
                        disabled={!editable}
                      />
                    </Field>
                    <Field label="E-mail">
                    ...
                    <Field label="Telefone">
                      <Input
                        value={forms[p].telefone ?? ""}
                        onChange={(e) => update(p, "telefone", e.target.value)}
                        disabled={!editable}
                      />
                    </Field>
                    <Field label="Endereço" colSpan={2}>
                      <Input
                        value={forms[p].endereco ?? ""}
                        onChange={(e) => update(p, "endereco", e.target.value)}
                        disabled={!editable}
                      />
                    </Field>
                    <Field label="Regime de casamento">
                      <Input
                        ...
                        onChange={(e) => update(p, "regime_casamento", e.target.value)}
                ...
                {p.startsWith("vendedor_") && (
                  <CardContent className="border-t pt-4">
                    ...
                    <FieldGrid>
                      <Field label="Banco">
                        <Input
                          value={bankForms[p]?.banco ?? ""}
                          onChange={(e) => updBank(p, "banco", e.target.value)}
                          disabled={!editable}
                        />
                      </Field>
                      <Field label="Agência">
                        <Input
                          value={bankForms[p]?.agencia ?? ""}
                          onChange={(e) => updBank(p, "agencia", e.target.value)}
                          disabled={!editable}
                        />
                      </Field>
                      <Field label="Conta">
                        <Input
                          value={bankForms[p]?.conta ?? ""}
                          onChange={(e) => updBank(p, "conta", e.target.value)}
                          disabled={!editable}
                        />
                      </Field>
                      <Field label="PIX">
                        <Input
                          value={bankForms[p]?.pix ?? ""}
                          onChange={(e) => updBank(p, "pix", e.target.value)}
                          disabled={!editable}
                        />
                      </Field>
                    </FieldGrid>
                ...
                <CardContent className="flex flex-wrap items-center justify-between gap-2 pt-0">
                  <div className="flex gap-2">
                    {editable && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => addPapel("comprador")}>
                          + Adicionar comprador
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => addPapel("vendedor")}>
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    {i > 0 && (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => setActivePapel(papeis[i - 1])}
                      >
