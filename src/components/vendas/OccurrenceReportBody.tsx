  ...
  return (
    <tr>
      {cols.map((c, i) => (
        <th
          key={i}
          className="border border-foreground/30 bg-muted/40 px-2 py-1 text-left text-[11px] font-semibold uppercase tracking-wide text-muted-foreground"
        >
          {c}
        </th>
      ))}
...
export function OccurrenceReportBody({
  sale,
  occ,
  commissions,
  partners,
  ...
  distribuicao?: DistributionSummary | null;
  papeis?: typeof COMISSAO_PAPEIS;
}) {
  ...
  const vendedores = Object.entries(parties)
    .filter(([papel]) => papel.startsWith("vendedor"))
  ...
  const valorImobiliaria =
    distribuicao?.saldo_liquido_imobiliaria ??
    distribuicao?.saldo_imobiliaria ??
    (() => {
      const baseImobiliaria =
        sale.percentual_remax != null ? Number(sale.valor_remax ?? 0) : valorNosso;
  ...
  return (
    <>
      ...
      <FormTable>
        ...
        <FormValueRow
          cols={[
            ...
            <Checkbox
              checked={!!occ?.nota_fiscal_obrigatoria}
              label={occ?.nota_fiscal_obrigatoria ? "Sim" : "Não"}
            />,
            occ?.midia,
          ]}
        />
      ...
      {vendedores.map((v, i) => (
        <FormTable key={v.id ?? i}>
          <FormValueRow
            cols={[
              <span>
                <b>Nome do vendedor/proprietário:</b> {v.nome}
              </span>,
              <span>
                <b>E-mail:</b> {v.email}
              </span>,
          ...
          <FormValueRow
            cols={[
              <span>
                <b>CPF/CNPJ:</b> {v.cpf_cnpj}
              </span>,
              <span>
                <b>RG:</b> {v.rg}
              </span>,
              <span>
                <b>Celular:</b> {v.telefone}
              </span>,
            ]}
          />
          <FormValueRow
      ...
      {compradores.map((c, i) => (
        <FormTable key={c.id ?? i}>
          ...
          <FormValueRow
            cols={[
              <span>
                <b>CPF/CNPJ:</b> {c.cpf_cnpj}
              </span>,
              <span>
                <b>RG:</b> {c.rg}
              </span>,
              <span>
                <b>Celular:</b> {c.telefone}
              </span>,
            ]}
          />
          <FormValueRow
      ...
      <FormTable>
        <FormHeadRow
          cols={["Valor anunciado", "Valor negociado", "Percentual", "Valor da comissão"]}
        />
        <FormValueRow
          cols={[
            money(occ?.valor_anunciado ?? sale.valor_anunciado),
            money(occ?.valor_negociado ?? sale.valor_negociado),
            (occ?.percentual_comissao ?? sale.percentual_comissao)
              ? `${occ?.percentual_comissao ?? sale.percentual_comissao}%`
              : null,
            <span className="text-base font-bold text-primary">
              {money(occ?.valor_comissao ?? sale.valor_total_comissao)}
            </span>,
          ]}
        />
        {somaParceria > 0 && (
      ...
      <FormTable>
        ...
        {sale.percentual_remax != null && (
          <FormValueRow
            cols={["REMAX", "—", `${sale.percentual_remax}%`, money(sale.valor_remax) ?? "R$ 0,00"]}
          />
        )}
        <FormValueRow
          cols={[<b>Imobiliária</b>, "—", "—", <b>{money(valorImobiliaria) ?? "R$ 0,00"}</b>]}
        />
      </FormTable>

      <FormTitle
        right={
          <Checkbox checked={!!occ?.financiamento} label={occ?.financiamento ? "Sim" : "Não"} />
        }
      >
      ...
      <FormTable>
        <FormHeadRow
          cols={[
            ...
            "Previsão da liberação do crédito",
            "Oba Crédito",
          ]}
        />
        <FormValueRow
          cols={[
            money(occ?.financiamento_valor),
            occ?.financiamento_banco,
            occ?.financiamento_correspondente,
            dateBR(occ?.financiamento_previsao),
            occ?.oba_credito ? "Sim" : "Não",
          ]}
        />
      </FormTable>
      ...
      <FormTable>
        ...
        <FormValueRow
          cols={[
            money(occ?.prev_recebimento_valor),
            dateBR(occ?.prev_recebimento_data),
            occ?.prev_recebimento_forma,
          ]}
        />
        <FormHeadRow cols={["2ª parcela", "Data", "Forma de pagamento"]} />
        <FormValueRow
          cols={[
            money(occ?.prev_recebimento2_valor),
            dateBR(occ?.prev_recebimento2_data),
            occ?.prev_recebimento2_forma,
          ]}
        />
        <FormHeadRow cols={["3ª parcela", "Data", "Forma de pagamento"]} />
        <FormValueRow
          cols={[
            money(occ?.prev_recebimento3_valor),
            dateBR(occ?.prev_recebimento3_data),
            occ?.prev_recebimento3_forma,
          ]}
        />
      </FormTable>
      ...
      <FormTable>
        ...
        />
        {partners.length === 0 && (
          <FormValueRow cols={["Não possui", null, null, "0%", "R$ 0,00"]} />
        )}
        {partners.map((p) => (
          <FormValueRow
            key={p.id}
