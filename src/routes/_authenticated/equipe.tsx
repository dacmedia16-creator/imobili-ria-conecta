...
function EquipesPage() {
  ...
  const membersByTeam = useMemo(() => {
    ...
    members.forEach((m) => {
      (map[m.team_id] ??= []).push(m.membro_id);
    });
    return map;
  }, [members]);
  ...
  const childrenByParent = useMemo(() => {
    ...
    teams.forEach((t) => {
      if (t.parent_team_id) (map[t.parent_team_id] ??= []).push(t);
    });
    return map;
  }, [teams]);
  ...
  const liderOptions = useMemo(
    () => (isAdminLike ? gestores : gestores.filter((g) => g.id === user?.id)),
    [isAdminLike, gestores, user?.id],
  );
  ...
  const filteredTop = useMemo(() => {
    ...
    return topLevel.filter(
      (t) =>
        ...
        (childrenByParent[t.id] ?? []).some((c) => c.nome.toLowerCase().includes(q)),
    );
    ...
    <div className="space-y-6">
      ...
      {desempenhoTeam && (
        <DesempenhoDialog
          ...
          onOpenChange={(open) => {
            if (!open) setDesempenhoTeam(null);
          }}
        />
      )}
    ...
    return Array.from(ids)
      .map((id) => {
        ...
        return {
          ...
          equipe:
            (comercial?.team_id && teamNameById.get(comercial.team_id)) ||
            teamNameByMembro[id] ||
            "Sem equipe",
          total: vendas.length,
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-5">
          <div>
            ...
            <p className="text-xl font-semibold">{totais.equipes}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-xs text-muted-foreground">VGV atribuído à REMAX</p>
    ...
    <Card>
      <CardContent className="space-y-3 pt-6">
        <TeamRow
          ...
          coLiderIds={coLeadersByTeam[team.id] ?? []}
        />
        {manageable ? (
          ...
          <p className="border-t pt-3 text-xs text-muted-foreground">
            Equipe-mãe — gerenciada por {profiles[team.lider_id]?.nome || "outro líder"}.
          </p>
        )}
            ...
            {subTeams.map((sub) => (
              <div key={sub.id} className="space-y-2 rounded-md border p-3">
                <TeamRow
                  team={sub}
                  profiles={profiles}
                  memberCount={(membersByTeam[sub.id] ?? []).length}
                  coLiderIds={coLeadersByTeam[sub.id] ?? []}
                  compact
                />
                {canManageTeam(sub) ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    Gerenciada por {profiles[sub.lider_id]?.nome || "outro líder"}.
                  </p>
                )}
              </div>
            ))}
...
function TeamRow({
  ...
  const coLideres = (coLiderIds ?? [])
    .map((id) => profiles[id]?.nome || profiles[id]?.email || id)
    .filter(Boolean);
  return (
...
function TeamFormDialog({
  ...
  const [liderId, setLiderId] = useState(
    state.initial?.lider_id ?? (gestores.length === 1 ? gestores[0].id : ""),
  );
  ...
  const title =
    state.mode === "edit"
      ...
      : state.mode === "create-sub"
        ? `Nova sub-equipe de "${state.parent?.nome}"`
        : "Nova equipe";
    ...
    try {
      if (state.mode === "edit" && state.initial) {
        const { error } = await supabase
          .from("teams")
          .update({ nome: nome.trim(), cor, lider_id: liderId })
          .eq("id", state.initial.id);
      ...
      toast.error(errorMessage(err, "Falha ao salvar equipe"));
    } finally {
      setSaving(false);
    }
  };
          ...
          <div>
            ...
            {gestores.length === 0 && (
              <p className="mt-1 text-xs text-muted-foreground">
                Nenhum usuário com papel gestor ou team leader cadastrado ainda.
              </p>
            )}
          </div>
          <DialogFooter>
...
function MembrosDialog({
  ...
  const carregarCandidatos = useCallback(async () => {
    ...
    try {
      ...
      toast.error(errorMessage(err, "Falha ao listar corretores"));
    } finally {
  ...
  const adicionar = async () => {
    ...
    try {
      const { error } = await supabase
        .from("team_members")
        .insert({ team_id: team.id, membro_id: selecionado });
      if (error) throw error;
      ...
      toast.error(errorMessage(err, "Falha ao adicionar corretor"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const remover = async (membroId: string) => {
    ...
    try {
      ...
      toast.error(errorMessage(err, "Falha ao remover corretor"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Membros — {team.nome}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1">
            {membroIds.length === 0 && (
              <p className="text-sm text-muted-foreground">Nenhum corretor nesta equipe ainda.</p>
            )}
            {membroIds.map((id) => (
              <div
                key={id}
          ...
          <div className="flex items-end gap-2 border-t pt-3">
            ...
            <Button onClick={adicionar} disabled={!selecionado || busy}>
              Adicionar
            </Button>
          </div>
        </div>
...
function CoLideresDialog({
  ...
  const adicionar = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({ team_id: team.id, user_id: selecionado });
      if (error) throw error;
      toast.success("Líder auxiliar adicionado");
      setSelecionado("");
      onChanged();
    } catch (err: unknown) {
      toast.error(errorMessage(err, "Falha ao adicionar líder auxiliar"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const remover = async (userId: string) => {
    ...
    try {
      ...
      toast.error(errorMessage(err, "Falha ao remover líder auxiliar"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Líderes auxiliares — {team.nome}</DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground">
          O líder auxiliar tem as mesmas permissões do líder principal (cadastrar corretor,
          aprovar/devolver venda, mandar pro jurídico etc.), mas só dentro desta equipe.
        </p>
        <div className="space-y-3">
          <div className="space-y-1">
            {coLiderIds.length === 0 && (
              <p className="text-sm text-muted-foreground">
                Nenhum líder auxiliar nesta equipe ainda.
              </p>
            )}
            {coLiderIds.map((id) => (
              <div
          ...
          <div className="flex items-end gap-2 border-t pt-3">
            <div className="flex-1">
              ...
              {candidatos.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Nenhum gestor/team leader disponível para adicionar.
                </p>
              ) : (
                <Select value={selecionado} onValueChange={setSelecionado}>
                  <SelectTrigger>
...
function MetaProgressBar({ realizado, meta }: { realizado: number; meta: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={`text-xs font-medium ${pct >= 100 ? "text-emerald-700 dark:text-emerald-400" : "text-muted-foreground"}`}
      >
        {pct}%
      </span>
    </div>
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Input
        ...
        placeholder={placeholder ?? "Definir meta"}
        value={draft}
...
function DesempenhoDialog({
  ...
  const carregarMetas = useCallback(async () => {
    ...
    setMetas((data as MetaProgresso | null) ?? { corretor: [], equipe: [] });
  }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data }, vendasValidas, atribuicoes] = await Promise.all([
        membroIds.length
          ? await supabase
              .from("sales")
              .select("id, corretor_id, status, valor_negociado, valor_total_comissao")
              .in("corretor_id", membroIds)
              .not("status", "in", "(cancelada,arquivada)")
          : Promise.resolve({ data: [] as PerformanceSale[] }),
        fetchVendasComerciaisValidas(),
        fetchAtribuicaoComercialResumo(),
      ...
      const idsValidos = new Set(vendasValidas.map((v) => v.sale_id));
      const salesData = (data ?? []).filter((s) => idsValidos.has(s.id));
      setSales(salesData);
      setAtribuicao(atribuicoes);
      await carregarMetas();
      setLoading(false);
  ...
  const salvarMeta = async (args: {
    tipo: "corretor" | "equipe";
    corretorId?: string;
    key: string;
    valor: number;
  }) => {
      ...
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            consideram o mês atual; vendas canceladas, arquivadas ou que voltaram de etapa não
            contam.
          </p>
        </DialogHeader>
        {loading ? (
            ...
            <div className="grid gap-3 sm:grid-cols-5">
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-xl font-semibold">{membroIds.length}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-xl font-semibold">
                    {money(metaEquipe?.comissao_realizada ?? 0)}
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
                  ...
                  {canManage ? (
                    ...
                    <p className="text-sm font-semibold">
                      {metaEquipe ? money(metaEquipe.meta_comissao) : "Sem meta"}
                    </p>
                  )}
                </CardContent>
              </Card>
              <Card>
                <CardContent className="flex flex-col justify-center pt-6">
                  ...
                  {metaEquipe ? (
                    <MetaProgressBar
                      realizado={metaEquipe.comissao_realizada}
                      meta={metaEquipe.meta_comissao}
                    />
                  ) : (
                      ...
                      <TableCell>
                        {canManage ? (
                          ...
                          <span className="text-sm text-muted-foreground">
                            {meta ? money(meta.meta_comissao) : "Sem meta"}
                          </span>
                        )}
                      </TableCell>
                      <TableCell>
                        {meta ? (
                          <MetaProgressBar
                            realizado={meta.comissao_realizada}
                            meta={meta.meta_comissao}
                          />
                        ) : (
