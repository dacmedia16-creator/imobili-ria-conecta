...
function AdminPositioning() {
  ...
  const load = async () => {
    const { data, error } = await supabase
      ...
      .select("id, suggested_by, nome, cidade, zona, tipo, created_at")
      .eq("status", "pendente")
      .order("created_at");
    if (error) {
  ...
  useEffect(() => {
    if (hasAny(["admin", "super_admin"])) load();
  }, [hasAny]);

  if (!hasAny(["admin", "super_admin"]))
    return (
      <p className="text-sm text-muted-foreground">
        Apenas administradores podem analisar sugestões.
      </p>
    );
  ...
  const review = async (decision: "aprovar" | "rejeitar", item: Suggestion) => {
    ...
    try {
      ...
      toast.success(
        decision === "aprovar" ? "Região aprovada e adicionada ao catálogo" : "Sugestão rejeitada",
      );
      setEditing(null);
      await load();
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-semibold tracking-tight">Sugestões de regiões</h1>
      </div>
      <p className="text-sm text-muted-foreground">
        Revise nomes para evitar duplicidades antes de disponibilizar a região para todos os
        corretores.
      </p>
      {items.length === 0 && (
        <Card>
            ...
            <Card key={item.id}>
              ...
              <CardContent className="space-y-4">
                <div className="text-xs text-muted-foreground">
                  Sugerido por <b>{names[item.suggested_by] ?? "Corretor"}</b> em{" "}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button onClick={() => review("aprovar", current)} disabled={saving}>
                    <Check className="mr-2 h-4 w-4" />
                    Aprovar
                  </Button>
                  <Button
                    variant="destructive"
                    onClick={() => review("rejeitar", item)}
                    disabled={saving}
                  >
