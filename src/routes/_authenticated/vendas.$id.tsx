...
import { STATUS_LABEL, DOC_TYPES, type SaleStatus } from "@/lib/status";
import { toast } from "sonner";
import { ArrowLeft, Upload, FileCheck, FileX, CheckCircle2, XCircle, Send, Gavel, DollarSign } from "lucide-react";
...
function SaleDetail() {
  ...
  // progress: simple fields filled count
  const fields = [sale.imovel_id, sale.matricula, sale.iptu, sale.valor_negociado, sale.percentual_comissao, sale.forma_pagamento, sale.posse_data, sale.comissao_valor];
  const progress = Math.round((fields.filter(Boolean).length / fields.length) * 100);

  const docsByType = (tipo: string) => docs.filter(d => d.tipo === tipo);
  const requiredTypes = DOC_TYPES.map(d => d.key);
  const docsApproved = requiredTypes.filter(t => docsByType(t).some(d => d.status === "aprovado")).length;
  ...
  const sendForReview = () => {
    if (docsApproved < 1 && docs.length === 0) {
      if (!confirm("Você ainda não enviou documentos. Deseja enviar mesmo assim?")) return;
    }
    changeStatus("enviada_revisao");
  };

  const returnForFix = async () => {
    const motivo = prompt("Motivo da devolução (obrigatório):");
    if (!motivo?.trim()) return;
    await changeStatus("devolvida_ajuste", motivo);
    await supabase.from("sale_comments").insert({ sale_id: id, autor_id: user!.id, escopo: "revisao", texto: motivo });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {isOwner && (status === "rascunho" || status === "devolvida_ajuste") && (
            <Button onClick={sendForReview}><Send className="mr-2 h-4 w-4" />Enviar para revisão</Button>
          )}
          {isGestor && status === "enviada_revisao" && (
            <>
              ...
              <Button variant="outline" onClick={returnForFix}><XCircle className="mr-2 h-4 w-4" />Devolver</Button>
            </>
          )}
          ...
          {isJuridico && status === "em_elaboracao_contrato" && (
            <Button onClick={() => changeStatus("aguardando_assinatura")}>Marcar “aguardando assinatura”</Button>
          )}
      ...
      <Tabs defaultValue="dados">
        ...
        <TabsContent value="dados" className="space-y-4 pt-4">
          ...
          <SaleSection title="Comissão">
            <FieldGrid>
              <Field label="Valor (R$)"><Input type="number" step="0.01" defaultValue={sale.comissao_valor ?? ""} disabled={!editable} onBlur={(e) => saveSale({ comissao_valor: e.target.value ? Number(e.target.value) : null })} /></Field>
              <Field label="Quando será paga"><Input defaultValue={sale.comissao_quando ?? ""} disabled={!editable} onBlur={(e) => saveSale({ comissao_quando: e.target.value || null })} /></Field>
              <Field label="Observações" colSpan={2}><Textarea defaultValue={sale.comissao_observacoes ?? ""} disabled={!editable} onBlur={(e) => saveSale({ comissao_observacoes: e.target.value || null })} /></Field>
            </FieldGrid>
          </SaleSection>
        </TabsContent>
        ...
        <TabsContent value="ocorrencia" className="space-y-4 pt-4">
          <OccurrencePanel saleId={id} sale={sale} canEdit={isFinanceiro || isGestor} onChange={load} />
        </TabsContent>
      </Tabs>
    ...
    <Card>
      ...
      <CardContent>
        <FieldGrid>
          ...
          <Field label="FGTS — observação" colSpan={2}><Input value={f.fgts_observacao ?? ""} onChange={(e) => upd("fgts_observacao", e.target.value)} disabled={!editable} /></Field>
          ...
          <Field label="Financiamento — observação" colSpan={2}><Input value={f.financiamento_observacao ?? ""} onChange={(e) => upd("financiamento_observacao", e.target.value)} disabled={!editable} /></Field>
...
function OccurrencePanel({ saleId, sale, canEdit, onChange }: { saleId: string; sale: any; canEdit: boolean; onChange: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setOcc(o);
    if (o) {
      const { data: c } = await supabase.from("occurrence_commissions").select("*").eq("occurrence_id", o.id);
      setCommissions(c ?? []);
    }
    setLoading(false);
  }, [saleId]);
  ...
  const createOcc = async () => {
    ...
    if (error) toast.error(error.message); else { setOcc(data); onChange(); }
  };
  ...
  const updCommission = async (id: string, patch: any) => {
    const { error } = await supabase.from("occurrence_commissions").update(patch).eq("id", id);
    if (error) toast.error(error.message); else load();
  };
  ...
  const conclude = async () => {
    const sum = commissions.reduce((s, c) => s + Number(c.valor ?? 0), 0);
    const total = Number(occ.valor_comissao ?? 0);
    if (total && sum > total + 0.01) {
      if (!confirm(`Soma das comissões (R$ ${sum.toFixed(2)}) excede a comissão total (R$ ${total.toFixed(2)}). Continuar mesmo assim?`)) return;
    }
    await updateOcc({ status: "concluida" });
    await supabase.from("sales").update({ status: "ocorrencia_concluida" }).eq("id", saleId);
    onChange();
  };
    ...
    return (
      <Card>
        <CardContent className="space-y-3 p-6 text-center">
          ...
          {canEdit && <Button onClick={createOcc}>Criar ocorrência a partir dos dados da venda</Button>}
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-base">Ocorrência de compra e venda</CardTitle>
          <span className="text-xs text-muted-foreground">{occ.status === "concluida" ? "Concluída" : "Pendente"}</span>
        </CardHeader>
        <CardContent>
          <FieldGrid>
            <Field label="Código do imóvel"><Input defaultValue={occ.codigo_imovel ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ codigo_imovel: e.target.value || null })} /></Field>
            <Field label="Data de assinatura"><Input type="date" defaultValue={occ.data_assinatura ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ data_assinatura: e.target.value || null })} /></Field>
            <Field label="Mídia"><Input defaultValue={occ.midia ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ midia: e.target.value || null })} /></Field>
            <Field label="Nota fiscal obrigatória"><div className="flex items-center gap-2"><Switch checked={!!occ.nota_fiscal_obrigatoria} onCheckedChange={(v) => updateOcc({ nota_fiscal_obrigatoria: v })} disabled={!canEdit} /></div></Field>
            <Field label="Valor anunciado"><Input type="number" step="0.01" defaultValue={occ.valor_anunciado ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ valor_anunciado: e.target.value ? Number(e.target.value) : null })} /></Field>
            <Field label="Valor negociado"><Input type="number" step="0.01" defaultValue={occ.valor_negociado ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ valor_negociado: e.target.value ? Number(e.target.value) : null })} /></Field>
            <Field label="% Comissão"><Input type="number" step="0.001" defaultValue={occ.percentual_comissao ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ percentual_comissao: e.target.value ? Number(e.target.value) : null })} /></Field>
            <Field label="Valor da comissão"><Input type="number" step="0.01" defaultValue={occ.valor_comissao ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ valor_comissao: e.target.value ? Number(e.target.value) : null })} /></Field>
            <Field label="Observações" colSpan={2}><Textarea defaultValue={occ.observacoes ?? ""} disabled={!canEdit} onBlur={(e) => updateOcc({ observacoes: e.target.value || null })} /></Field>
          </FieldGrid>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-base">Comissões detalhadas</CardTitle>
          {canEdit && <Button size="sm" onClick={addCommission}>Adicionar comissão</Button>}
        </CardHeader>
        <CardContent className="space-y-2">
          {commissions.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma comissão adicionada.</p>}
          {commissions.map((c) => (
            <div key={c.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-5">
              <Select value={c.papel} onValueChange={(v) => updCommission(c.id, { papel: v })} disabled={!canEdit}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="corretor_captador">Corretor captador</SelectItem>
                  <SelectItem value="indicador_captador">Indicador captador</SelectItem>
                  <SelectItem value="coordenador_captador">Coordenador captador</SelectItem>
                  <SelectItem value="corretor_vendedor">Corretor vendedor</SelectItem>
                  <SelectItem value="indicador_vendedor">Indicador vendedor</SelectItem>
                  <SelectItem value="coordenador_vendedor">Coordenador vendedor</SelectItem>
                </SelectContent>
              </Select>
              <Input placeholder="Nome" defaultValue={c.nome ?? ""} onBlur={(e) => updCommission(c.id, { nome: e.target.value || null })} disabled={!canEdit} />
              <Input type="number" step="0.001" placeholder="%" defaultValue={c.percentual ?? ""} onBlur={(e) => updCommission(c.id, { percentual: e.target.value ? Number(e.target.value) : null })} disabled={!canEdit} />
              <Input type="number" step="0.01" placeholder="Valor" defaultValue={c.valor ?? ""} onBlur={(e) => updCommission(c.id, { valor: e.target.value ? Number(e.target.value) : null })} disabled={!canEdit} />
              {canEdit && <Button variant="ghost" size="sm" onClick={() => delCommission(c.id)}>Remover</Button>}
            </div>
          ))}
        </CardContent>
      </Card>

      {canEdit && occ.status !== "concluida" && (
        <Button onClick={conclude}>Concluir ocorrência</Button>
      )}
    </div>
  );
}
