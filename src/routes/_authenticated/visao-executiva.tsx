...
function VisaoExecutiva() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Precisa de ação</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <AlertaCard icon={FileSignature} label="Aguardando assinatura há +7 dias" alerta={stats?.alertas?.assinatura_pendente} critical />
...
function AlertaCard({ icon: Icon, label, alerta, critical, sub }: { icon: any; label: string; alerta?: Alerta; critical?: boolean; sub?: string }) {
  const n = alerta?.n ?? 0;
  const isZero = n === 0;
  return (
    <Card className={!isZero && critical ? "border-destructive/40" : ""}>
      <CardContent className="space-y-2 p-4">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-2xl font-bold leading-none ${isZero ? "text-muted-foreground/50" : ""}`}>{n}</span>
        </div>
