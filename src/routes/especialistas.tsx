...
function SpecialistsPage() {
  ...
  useEffect(() => {
    supabase.rpc("list_public_positioning_regions").then(({ data, error: catalogError }) => {
      if (catalogError) {
        setError("Não foi possível carregar as regiões agora.");
        return;
      }
      setRegions(data ?? []);
    });
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      if (specialistsError) {
        ...
        setSpecialists(
          (data ?? []).map((row) => ({ ...row, regioes: parseSpecialistRegions(row.regioes) })),
        );
      }
      setLoading(false);
    }, 250);
      ...
      <main>
        <section className="mx-auto max-w-4xl px-4 pb-12 pt-8 text-center text-white sm:px-6 sm:pt-14">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm text-blue-100 sm:text-base">
            Busque pelo bairro, condomínio, cidade ou nome do corretor e fale diretamente com quem
            conhece a região.
          </p>
              ...
              <SelectContent>
                ...
                {regions.map((region) => (
                  <SelectItem key={region.id} value={String(region.id)}>
                    {region.nome} — {region.cidade}
                    {region.zona ? ` / ${region.zona}` : ""}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="mx-auto max-w-7xl">
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">
                  {activeRegion
                    ? `Especialistas em ${activeRegion.nome}`
            ...
            {!error && !loading && specialists.length === 0 && (
              <div className="rounded-2xl border border-dashed bg-white p-10 text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Tente outra região ou faça uma busca mais ampla.
                </p>
              </div>
            )}
                ...
                <Card key={specialist.id} className="overflow-hidden border-0 shadow-md">
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-16 w-16 ring-2 ring-blue-100">
                        <AvatarImage
                          ...
                          alt={specialist.nome}
                        />
                        <AvatarFallback className="bg-[#123a7a] text-lg text-white">
                          {specialist.nome[0]?.toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div className="min-w-0">
                        ...
                        <p className="text-xs font-medium uppercase tracking-wide text-[#1a58a8]">
                    ...
                    <div className="mt-5 space-y-2 text-sm text-slate-600">
                      {specialist.telefone && (
                        <a
                          href={whatsappSpecialistUrl(specialist)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 hover:text-[#1a58a8] hover:underline"
                        >
                      ...
                      {specialist.pagina_pessoal_url && (
                        <a
                          href={specialist.pagina_pessoal_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 hover:text-[#1a58a8] hover:underline"
                        >
