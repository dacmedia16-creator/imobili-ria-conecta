...
export function getSaleRoleFlags(roles: AppRole[], saleCorretorId: string, userId: string | undefined) {
  return {
    isOwner: saleCorretorId === userId,
    isFinanceiro: roles.some((r) => (["financeiro", "admin", "super_admin"] as AppRole[]).includes(r)),
...
export function gestorPodeEditar(isGestor: boolean, status: SaleStatus): boolean {
  return isGestor && (
    ["enviada_revisao", "contrato_conferencia_gestor", "contrato_ok_corretor", "aguardando_assinatura", "contrato_assinado", "ocorrencia_pendente", "ocorrencia_devolvida_gestor"] as SaleStatus[]
  ).includes(status);
}

export function juridicoPodeEditar(isJuridico: boolean, status: SaleStatus): boolean {
  return isJuridico && (["aprovada_gestor", "em_elaboracao_contrato"] as SaleStatus[]).includes(status);
}

/** Quem pode editar campos (Resumo/Partes/Pagamento/Docs) segundo o estado atual. */
export function podeEditarVenda(args: {
  corretorEdits: boolean; gestorEdits: boolean; juridicoEdits: boolean;
  isFinanceiro: boolean; isAdminLike: boolean; locked: boolean;
}): boolean {
  return (args.corretorEdits || args.gestorEdits || args.juridicoEdits || args.isFinanceiro || args.isAdminLike)
...
export function podeEditarComissao(args: {
  gestorEdits: boolean; isFinanceiro: boolean; isAdminLike: boolean; locked: boolean;
}): boolean {
  return (args.gestorEdits || args.isFinanceiro || args.isAdminLike) && (!args.locked || args.isFinanceiro || args.isAdminLike);
}
...
export function comissaoValorExcedido(valorTotalComissao: number, valorCaptador: number, valorVendedor: number): boolean {
...
export function podeVerOcorrencia(status: SaleStatus): boolean {
  return (["contrato_assinado", "ocorrencia_pendente", "ocorrencia_analise_financeiro", "ocorrencia_devolvida_gestor", "ocorrencia_concluida"] as SaleStatus[]).includes(status);
}
...
export function podeEditarOcorrencia(args: { isGestor: boolean; status: SaleStatus; isFinanceiro: boolean; isAdminLike: boolean }): boolean {
  return (args.isGestor && (["contrato_assinado", "ocorrencia_pendente", "ocorrencia_devolvida_gestor"] as SaleStatus[]).includes(args.status))
    || args.isFinanceiro || args.isAdminLike;
}
