...
describe("calcularPatchValorNegociado — comissão em reais como fonte oficial", () => {
  ...
  it("nunca toca indicador ou líder — são valores fixos em reais descontados do próprio lado, não do negociado", () => {
    const formSale = {
      ...
      valor_comissao_lider_captador: 1000,
    };
...
describe("calcularPatchOccValorNegociado — valor em reais prevalece", () => {
  it("preserva o valor da comissão e deriva o percentual", () => {
    const patch = calcularPatchOccValorNegociado(
      { percentual_comissao: 4.844, valor_comissao: 32700 },
      675000,
    );
  ...
  it("sem percentual definido, não mexe em valor_comissao", () => {
    const patch = calcularPatchOccValorNegociado(
      { percentual_comissao: null, valor_comissao: null },
      730000,
    );
    expect(patch).not.toHaveProperty("valor_comissao");
  });
...
describe("verificarComissoesDesatualizadas", () => {
  ...
  const sale = {
    ...
    corretor_captador_id: "captador-1",
    valor_comissao_captador: 4927.5,
    corretor_vendedor: null,
    corretor_vendedor_id: null,
    valor_comissao_vendedor: null,
    ...
    lider_captador_nome: null,
    valor_comissao_lider_captador: null,
    lider_captador_id: null,
    lider_vendedor_nome: null,
    valor_comissao_lider_vendedor: null,
    lider_vendedor_id: null,
  };
  const distribuicao = { liquido_captador: 4927.5, liquido_vendedor: null };
  const linhaCaptadorOk: OcorrenciaComissaoRow = {
  ...
  const linhaManual: OcorrenciaComissaoRow = {
    papel: "corretor_captador",
  ...
  it("teste 1 — linha automática correta + linha manual do mesmo papel: aviso falso, manual nunca usada na comparação", () => {
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaManual],
      commissionExtras: [],
    });
  ...
  it("teste 2 — linha manual aparece ANTES da automática no array: ainda encontra a automática", () => {
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaManual, linhaCaptadorOk],
      commissionExtras: [],
    });
    expect(desatualizado).toBe(false);
  });
  ...
  it("teste 3 — indicador removido do Resumo mas linha automática antiga ainda presente: aviso verdadeiro", () => {
    const linhaIndicadorVelha: OcorrenciaComissaoRow = {
    ...
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaIndicadorVelha],
      commissionExtras: [],
    });
  ...
  it("teste 5 — líder removido com linha automática antiga: aviso verdadeiro", () => {
    const linhaLiderVelha: OcorrenciaComissaoRow = {
    ...
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaLiderVelha],
      commissionExtras: [],
    });
    expect(desatualizado).toBe(true);
  });
  ...
  it("teste 6 — captador líquido correto com linha manual adicional (extra independente): aviso falso", () => {
    const linhaManualOutroPapel: OcorrenciaComissaoRow = {
      papel: "outro",
      nome: "Ajuste avulso do financeiro",
      valor: 50,
      user_id: null,
      sale_commission_extra_id: null,
      managed_by_sale: false,
    ...
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaManualOutroPapel],
      commissionExtras: [],
    });
    expect(desatualizado).toBe(false);
  });
  ...
  it("teste 9 — extra removido do Resumo mas linha ainda presente na Ocorrência: aviso verdadeiro", () => {
    const linhaExtraOrfa: OcorrenciaComissaoRow = {
      papel: "outro",
      nome: "Extra removido",
      valor: 200,
      user_id: null,
      sale_commission_extra_id: "extra-removido-1",
      managed_by_sale: true,
    };
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaExtraOrfa],
      commissionExtras: [],
    });
    expect(desatualizado).toBe(true);
  });
  ...
  it("teste 10 — extra correto e uma linha manual independente: aviso falso", () => {
    ...
    const linhaExtraOk: OcorrenciaComissaoRow = {
      papel: "outro",
      nome: "Extra Correto",
      valor: 200,
      user_id: null,
    ...
    const desatualizado = verificarComissoesDesatualizadas({
      sale,
      distribuicao,
      commissions: [linhaCaptadorOk, linhaExtraOk, linhaManual],
      commissionExtras: [extra],
    });
    expect(desatualizado).toBe(false);
  });
