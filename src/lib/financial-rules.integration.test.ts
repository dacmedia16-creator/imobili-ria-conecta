...
describe.skipIf(!HAS_SUPABASE_ADMIN_ENV)("Regras financeiras (integração via RPCs reais)", () => {
  ...
  beforeAll(async () => {
    ...
    if (PROFILES.length < 5) {
      throw new Error(
        "Preciso de pelo menos 5 profiles cadastrados pra rodar a suíte de integração das regras financeiras.",
      );
    }
  }, 30000);
  ...
  async function criarExtra(saleId: string, overrides: Record<string, any>) {
    const { error } = await supabaseAdmin
      ...
      .insert({ sale_id: saleId, ...overrides });
    if (error) throw error;
  }

  async function distribuicao(saleId: string) {
    const { data, error } = await supabaseAdmin.rpc("calcular_distribuicao_venda", {
      p_sale_id: saleId,
    });
    if (error) throw error;
  ...
  async function criarOcorrenciaCompleta(saleId: string) {
    const { data, error } = await supabaseAdmin.rpc("criar_ocorrencia_completa", {
      p_sale_id: saleId,
    });
    if (error) throw error;
  ...
  async function metasProgressoCorretor(
    userId: string,
    mes = new Date().toISOString().slice(0, 8) + "01",
  ) {
    const { data, error } = await supabaseAdmin.rpc("metas_progresso", { _mes: mes });
    if (error) throw error;
    const row = ((data as unknown as MetasProgresso).corretor ?? []).find(
      (r) => r.corretor_id === userId,
    );
  ...
  it("regra 4 — captador e vendedor são valores livres em reais, nunca um percentual fixo", async () => {
    const saleAId = await criarVenda({
      ...CENARIO_BASE,
      valor_comissao_captador: 3000,
      valor_comissao_vendedor: 7000,
    });
    const saleBId = await criarVenda({
      ...CENARIO_BASE,
      valor_comissao_captador: 9500,
      valor_comissao_vendedor: 500,
    });
  ...
  it("regra 10 — remover um extra depois de sincronizado remove a linha correspondente em occurrence_commissions", async () => {
    ...
    const antes = await supabaseAdmin
      ...
      .select("id, valor")
      .eq("sale_commission_extra_id", extraId)
      .maybeSingle();
  ...
  it("regra proteção — linha manual (managed_by_sale=false) com o mesmo papel do captador não é sobrescrita pela sincronização", async () => {
    const saleId = await criarVenda({
      ...
      corretor_captador: "Captador Real",
    });
    ...
    const { error: manualErr } = await supabaseAdmin.from("occurrence_commissions").insert({
      ...
      nome: "Ajuste Manual do Financeiro",
      valor: 999.99,
    ...
    const { data: linhas } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("nome, valor, managed_by_sale")
      .eq("occurrence_id", occId)
      .eq("papel", "corretor_captador");
  ...
  it("regra proteção — linha manual sobrevive mesmo quando a venda não tem captador algum (ramo de remoção da RPC)", async () => {
    ...
    const { error: manualErr } = await supabaseAdmin.from("occurrence_commissions").insert({
      occurrence_id: occId,
      papel: "corretor_captador",
      nome: "Ajuste Manual Sem Captador",
      valor: 500,
      managed_by_sale: false,
    });
    if (manualErr) throw manualErr;
  ...
  it("regra idempotência — chamar sync_occurrence_commissions várias vezes seguidas não duplica nem altera nada", async () => {
    ...
    const { data: primeira } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("id, papel, valor, user_id, managed_by_sale")
      .eq("occurrence_id", occId)
      .order("papel");

    await sync(saleId);
    await sync(saleId);
    const { data: terceira } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("id, papel, valor, user_id, managed_by_sale")
      .eq("occurrence_id", occId)
      .order("papel");
  ...
  it("regra arredondamento — cálculo em JS (sale-financial-calc) e no Postgres (calcular_distribuicao_venda) batem centavo a centavo", async () => {
    ...
    const patchJs = calcularPatchValorNegociado(
      {
        percentual_comissao: percentualComissao,
        parceria_percentual: null,
        percentual_remax: null,
        valor_negociado: null,
        valor_total_comissao: null,
      },
      negociado,
    );
  ...
  it("regra 12 — venda cancelada não entra no ranking da Visão Executiva mesmo com comissão sincronizada", async () => {
    ...
    const saleId = await criarVenda({
      ...
      corretor_captador: "Captador Cancelado",
    });
  ...
  it("regra 13 — venda arquivada não entra no ranking da Visão Executiva mesmo com comissão sincronizada", async () => {
    ...
    const saleId = await criarVenda({
      ...
      status: "arquivada",
      corretor_captador_id: captadorId,
      corretor_captador: "Captador Arquivado",
    });
    await fecharVendaNoPeriodo(saleId);
    await criarOcorrencia(saleId, { valor_comissao: 43800 });
    await sync(saleId);
  ...
  it("regra 14 — venda cancelada e reaberta (status atual voltou a ativo) volta a contar no ranking", async () => {
    ...
    const saleId = await criarVenda({
      ...
      status: "contrato_assinado",
      corretor_captador_id: captadorId,
      corretor_captador: "Captador Reaberto",
    });
    // Histórico real de uma reabertura: cancelou há 10 dias, reabriu e assinou de novo há 2 dias.
    const cancelou = new Date();
    cancelou.setDate(cancelou.getDate() - 10);
    await supabaseAdmin
      .from("sale_status_history")
      .insert({ sale_id: saleId, para: "cancelada", created_at: cancelou.toISOString() });
  ...
  it("regra vínculo — captador/vendedor com nome mas sem conta vinculada é sinalizado, e some quando vinculado", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador: "Fulano Sem Conta",
      corretor_vendedor: "Beltrana Sem Conta",
    });
    const semVinculo = await distribuicao(saleId);
    expect(semVinculo.calculo_valido).toBe(false);
    expect(
      semVinculo.inconsistencias.some(
        (m: string) => m === 'Captador "Fulano Sem Conta" sem conta vinculada no sistema.',
      ),
    ).toBe(true);
    expect(
      semVinculo.inconsistencias.some(
        (m: string) => m === 'Vendedor "Beltrana Sem Conta" sem conta vinculada no sistema.',
      ),
    ).toBe(true);
    ...
    await supabaseAdmin
      .from("sales")
      .update({ corretor_captador_id: PROFILES[1], corretor_vendedor_id: PROFILES[2] })
      .eq("id", saleId);
    const comVinculo = await distribuicao(saleId);
    expect(comVinculo.inconsistencias.some((m: string) => m.includes("sem conta vinculada"))).toBe(
      false,
    );
  });
  ...
  it('regra vínculo — gestor e "outro captador" (extras) sem conta vinculada também são sinalizados', async () => {
    ...
    await criarExtra(saleId, {
      ...
      origem: "imobiliaria",
      valor: 1000,
      nome: "Gestor Sem Conta",
    });
    await criarExtra(saleId, {
      papel: "corretor_captador",
      origem: "imobiliaria",
      valor: 500,
      nome: "Outro Captador Sem Conta",
    });
    const dist = await distribuicao(saleId);
    expect(dist.calculo_valido).toBe(false);
    expect(
      dist.inconsistencias.some(
        (m: string) => m === 'Gestor "Gestor Sem Conta" sem conta vinculada no sistema.',
      ),
    ).toBe(true);
    expect(
      dist.inconsistencias.some(
        (m: string) =>
          m ===
          'Outro corretor captador "Outro Captador Sem Conta" sem conta vinculada no sistema.',
      ),
    ).toBe(true);
  });
  ...
  it("regra vínculo — Parceria Externa continua podendo ficar só no nome, sem conta (é o único caso permitido)", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      parceria_nome: "Imobiliária Parceira Ltda",
    });
    const dist = await distribuicao(saleId);
  ...
  it("regra 16 — parceria maior que a comissão bruta é sinalizada como inconsistência, não aceita silenciosamente", async () => {
    const saleId = await criarVenda({
      ...
      percentual_comissao: 6,
      parceria_tipo: "imobiliaria_externa",
      parceria_valor: 60000,
    });
    const dist = await distribuicao(saleId);
    expect(dist.calculo_valido).toBe(false);
    expect(
      dist.inconsistencias.some((m: string) =>
        m.includes("parceria externa ultrapassa a comissão bruta"),
      ),
    ).toBe(true);
  });
  ...
  it("regra 17a — indicador maior que a comissão do captador deixa o líquido negativo e é sinalizado", async () => {
    const saleId = await criarVenda({
      valor_negociado: 730000,
      valor_comissao_captador: 1000,
      valor_comissao_indicador_captador: 1500,
    });
    const dist = await distribuicao(saleId);
    expect(dist.liquido_captador).toBe(-500);
    expect(dist.calculo_valido).toBe(false);
    expect(
      dist.inconsistencias.some((m: string) => m.includes("líquido do captador ficou negativo")),
    ).toBe(true);
  });
  ...
  it("regra 17b — gestor/extras maiores que o saldo da imobiliária deixam o saldo líquido negativo e é sinalizado", async () => {
    ...
    await criarExtra(saleId, {
      ...
      origem: "imobiliaria",
      valor: 20000,
      nome: "Gestor Excedente",
    });
    const dist = await distribuicao(saleId);
    expect(dist.saldo_liquido_imobiliaria).toBe(12045 - 20000);
    expect(dist.calculo_valido).toBe(false);
    expect(
      dist.inconsistencias.some((m: string) => m.includes("ultrapassam o saldo disponível")),
    ).toBe(true);
  });
  ...
  it("regra 19 — quem criou a venda no sistema não recebe a comissão de quem participou de fato", async () => {
    ...
    const [criadorAntes, captadorAntes, vendedorAntes] = await Promise.all([
      comissaoRankingCorretor(criadorId),
    ...
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_id: criadorId,
      corretor_captador_id: captadorId,
      corretor_captador: "Captador Teste",
      corretor_vendedor_id: vendedorId,
      corretor_vendedor: "Vendedor Teste",
    });
    await fecharVendaNoPeriodo(saleId);
    await criarOcorrencia(saleId, { valor_comissao: 43800 });
    await sync(saleId);
  ...
  it("regra 20a — team_members.membro_id é único: um segundo vínculo pro mesmo corretor é rejeitado pelo banco", async () => {
    ...
    const { error: dupError } = await supabaseAdmin
      .from("team_members")
      .insert({ membro_id: existente.membro_id, team_id: existente.team_id, tipo: "membro" });
  ...
  it("regra 20b — a comissão do corretor cai inteira e uma única vez na equipe dele, sem duplicar", async () => {
    ...
    const [corretorAntes, equipeAntes] = await Promise.all([
      comissaoRankingCorretor(membro.membro_id),
      comissaoRankingEquipe(membro.team_id),
    ]);

    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: membro.membro_id,
      corretor_captador: "Membro de Equipe Teste",
    });
    await fecharVendaNoPeriodo(saleId);
    await criarOcorrencia(saleId, { valor_comissao: 43800 });
    await sync(saleId);
  ...
  it("regra equipe — 2 participantes da mesma equipe na mesma venda contam a venda 1 vez só pra equipe", async () => {
    ...
    for (const m of membros ?? [])
      porEquipe.set(m.team_id, [...(porEquipe.get(m.team_id) ?? []), m.membro_id]);
    ...
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: membroA,
      corretor_captador: "Membro A Teste",
      corretor_vendedor_id: membroB,
      corretor_vendedor: "Membro B Teste",
    });
    await fecharVendaNoPeriodo(saleId);
    await criarOcorrencia(saleId, { valor_comissao: 43800 });
    await sync(saleId);
  ...
  it("regra equipe — participante sem equipe (team_id null) continua aparecendo no ranking, não some", async () => {
    ...
    const [corretorAAntes, corretorBAntes, semEquipeVendasAntes, semEquipeComissaoAntes] =
      await Promise.all([
        comissaoRankingCorretor(semEquipeA),
        comissaoRankingCorretor(semEquipeB),
        vendasFechadasRankingEquipe(null as unknown as string),
        comissaoRankingEquipe(null as unknown as string),
      ]);
    ...
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: semEquipeA,
      corretor_captador: "Sem Equipe A",
      corretor_vendedor_id: semEquipeB,
      corretor_vendedor: "Sem Equipe B",
    });
    await fecharVendaNoPeriodo(saleId);
    await criarOcorrencia(saleId, { valor_comissao: 43800 });
    await sync(saleId);

    const [corretorADepois, corretorBDepois, semEquipeVendasDepois, semEquipeComissaoDepois] =
      await Promise.all([
        comissaoRankingCorretor(semEquipeA),
        comissaoRankingCorretor(semEquipeB),
        vendasFechadasRankingEquipe(null as unknown as string),
  ...
  it("teste 1 — criação com indicador: captador líquido = bruto − indicador", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 1",
      valor_comissao_indicador_captador: 500,
    });
  ...
  it("teste 2 — criação com indicador + extra do captador: cenário oficial da auditoria (4.227,50)", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 2",
      valor_comissao_indicador_captador: 500,
    });
    await criarExtra(saleId, {
      papel: "outro",
  ...
  it("teste 3 — vendedor com indicador + extra segue a mesma reconstrução do bruto", async () => {
    const saleId = await criarVenda({
      ...
      corretor_vendedor: "Vendedor Teste 3",
      valor_comissao_indicador_vendedor: 300,
    });
    await criarExtra(saleId, {
      papel: "outro",
  ...
  it("teste 4 — sem indicador nem extra, líquido = bruto", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 4",
    });
    const occId = await criarOcorrenciaCompleta(saleId);
    const { data: linha } = await supabaseAdmin
      .from("occurrence_commissions")
  ...
  it("teste 6 — pullFromSaleSplit (sync_occurrence_commissions) mantém captador/vendedor líquidos ao ressincronizar", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 6",
      valor_comissao_indicador_captador: 500,
    });
    const occId = await criarOcorrenciaCompleta(saleId);
  ...
  it("teste 6b — criar_ocorrencia_completa é idempotente e não duplica a ocorrência", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 6b",
    });
  ...
  it("teste 11 — depois de 'Puxar da revisão do gestor', o aviso de desatualização some e linhas manuais continuam intactas", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 11",
    });
    const occId = await criarOcorrenciaCompleta(saleId);
    const { error: manualErr } = await supabaseAdmin.from("occurrence_commissions").insert({
      occurrence_id: occId,
      papel: "outro",
      nome: "Ajuste Manual Teste 11",
      valor: 321,
      managed_by_sale: false,
    });
    if (manualErr) throw manualErr;
    ...
    await supabaseAdmin.from("sale_commission_extras").insert({
      sale_id: saleId,
      papel: "gestor",
      origem: "imobiliaria",
      valor: 1000,
      nome: "Gestor Puxado Depois",
      user_id: PROFILES[4],
    });
    ...
    const dist = await distribuicao(saleId);
    const { data: linhasFinal } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("papel, nome, valor, user_id, sale_commission_extra_id, managed_by_sale")
      .eq("occurrence_id", occId);
    const { data: extrasFinal } = await supabaseAdmin
      .from("sale_commission_extras")
      .select("id, papel, nome, valor, user_id")
      .eq("sale_id", saleId);
    const { data: saleAtual, error: saleErr } = await supabaseAdmin
      .from("sales")
      .select("*")
      .eq("id", saleId)
      .single();
    ...
    const desatualizado = verificarComissoesDesatualizadas({
      sale: saleAtual,
      distribuicao: dist as never,
      commissions: (linhasFinal ?? []) as never,
      commissionExtras: (extrasFinal ?? []) as never,
    });
  ...
  it("teste 8 — remover o extra do captador restaura o líquido pro bruto (menos só o indicador)", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 8",
      valor_comissao_indicador_captador: 500,
    });
    ...
    const extraLinha = await supabaseAdmin
      .from("occurrence_commissions")
      .select("id")
      .eq("sale_commission_extra_id", extraRow.id)
      .maybeSingle();
  ...
  it("teste 10 — falha de validação na criação não deixa Ocorrência órfã (rollback transacional)", async () => {
    ...
    const saleId = await criarVenda({
      ...
      valor_comissao_captador: 5000,
      valor_comissao_vendedor: 5000,
    });
  ...
  it("teste 12 — cenário financeiro oficial completo via RPC transacional (venda R$730.000)", async () => {
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: PROFILES[1],
      corretor_captador: "Captador Teste 12",
      corretor_vendedor_id: PROFILES[2],
      corretor_vendedor: "Vendedor Teste 12",
      valor_comissao_indicador_captador: 500,
      valor_comissao_indicador_vendedor: 300,
    });
    await criarExtra(saleId, {
  ...
  it("regra 21 — Resumo/Ocorrência (RPC) e Visão Executiva (ranking) concordam sobre a mesma venda", async () => {
    ...
    const saleId = await criarVenda({
      ...CENARIO_BASE,
      corretor_captador_id: captadorId,
      corretor_captador: "Captador Consistência",
      corretor_vendedor_id: vendedorId,
      corretor_vendedor: "Vendedor Consistência",
    });
    await criarExtra(saleId, {
      papel: "gestor",
      origem: "imobiliaria",
      valor: 1000,
      nome: "Gestor Consistência",
      user_id: PROFILES[4],
    });
    ...
    const { data: linhaCaptador } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("valor")
      .eq("occurrence_id", occId)
      .eq("papel", "corretor_captador")
      .single();
    const { data: linhaVendedor } = await supabaseAdmin
      .from("occurrence_commissions")
      .select("valor")
      .eq("occurrence_id", occId)
      .eq("papel", "corretor_vendedor")
      .single();
