...
const { PGlite } = await import(
  pathToFileURL(
    process.env.PGLITE_MODULE ||
      resolve(catalogDir, "sql-test/node_modules/@electric-sql/pglite/dist/index.js"),
  ).href
);
const db = new PGlite();
let checks = 0;
...
const baseMigrationPath = resolve(
  repo,
  "supabase/migrations/20260909193000_relatorio_ocorrencias_concluidas.sql",
);
const migrationPath = resolve(
  repo,
  "supabase/migrations/20260910230000_relatorio_ocorrencias_concluidas_participantes.sql",
);
...
const enumValues = (name) =>
  [...constants.match(new RegExp(`${name}: \\[([\\s\\S]*?)\\]`))[1].matchAll(/"([^"]+)"/g)].map(
    (m) => m[1],
  );
const allowedRoles = [
  "corretor",
...
for (const role of allowedRoles)
  check(enumValues("app_role").includes(role), `versioned role ${role}`);
check(
  !columns.some((c) => /tenant|organization|company|imobiliaria_id/.test(c.column_name)),
  "no tenant key in collected source tables",
);
...
async function loadVersionedFunction(name, filename) {
  ...
  const end =
    part.indexOf(delimiter, part.indexOf(delimiter) + delimiter.length) + delimiter.length;
...
const rpc = (actor) =>
  asActor(
    actor,
    async () =>
      (await db.query("SELECT public.relatorio_ocorrencias_concluidas() AS payload")).rows[0]
        .payload,
  );
...
async function state() {
  return {
    ...
    relations: (
      await db.query(
        "SELECT relname, relrowsecurity, relforcerowsecurity, relacl FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND relkind='r' ORDER BY relname",
      )
    ).rows,
    helpers: (
      await db.query(
        "SELECT p.proname, pg_get_functiondef(p.oid) AS definition, p.proacl FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN ('public','auth') AND p.proname <> 'relatorio_ocorrencias_concluidas' ORDER BY n.nspname, p.proname",
      )
    ).rows,
    data: await Promise.all(
      tables.map(async (t) => [
        ...
        (
          await db.query(
            `SELECT to_jsonb(t) AS row FROM public.${ident(t)} t ORDER BY to_jsonb(t)::text`,
          )
        ).rows,
      ]),
    ),
...
try {
  ...
  for (const table of tables) {
    ...
    await db.exec(
      `CREATE TABLE public.${ident(table)} (${defs.join(",")}); ALTER TABLE public.${ident(table)} ENABLE ROW LEVEL SECURITY; GRANT SELECT, INSERT, UPDATE, DELETE ON public.${ident(table)} TO authenticated;`,
    );
  }
  ...
  await db.exec(
    "CREATE TABLE public.sale_commission_extras(sale_id uuid,user_id uuid); ALTER TABLE public.sale_commission_extras ENABLE ROW LEVEL SECURITY; GRANT SELECT ON public.sale_commission_extras TO authenticated; SET check_function_bodies=off;",
  );
  ...
  for (const [name, filename] of [
    ...
    if (!effectiveFunctions.some((f) => f.proname === name))
      await loadVersionedFunction(name, filename);
  await db.exec("SET check_function_bodies=on");
  for (const p of policies) {
    assert.equal(p.schemaname, "public");
    const roles = p.roles.slice(1, -1).split(",").map(ident).join(",");
    await db.exec(
      `CREATE POLICY ${ident(p.policyname)} ON public.${ident(p.tablename)} AS ${p.permissive} FOR ${p.cmd} TO ${roles}${p.qual ? ` USING (${p.qual})` : ""}${p.with_check ? ` WITH CHECK (${p.with_check})` : ""};`,
    );
  }
  const actors = allowedRoles.map((role, i) => ({ name: role, id: uuid(i + 1) }));
  actors.push(
    { name: "inactive", id: uuid(20) },
    { name: "missing-profile", id: uuid(21) },
    { name: "no-role", id: uuid(22) },
    { name: "null-active", id: uuid(23) },
    { name: "legacy-coordenador", id: uuid(24) },
    { name: "null-uid", id: null },
    { name: "anon", id: null, dbRole: "anon" },
    { name: "public-execute", id: uuid(1), dbRole: "public_probe" },
  );
  for (const [i, actor] of actors.entries()) {
    if (!actor.id || actor.name === "missing-profile" || actor.name === "public-execute") continue;
    await db.query(
      "INSERT INTO public.profiles(id,nome,ativo,email,telefone) VALUES ($1,$2,$3,$4,$5)",
      [
        actor.id,
        `Pessoa sintética ${i}`,
        actor.name === "null-active" ? null : actor.name !== "inactive",
        "SECRET_EMAIL",
        "SECRET_PHONE",
      ],
    );
  }
  for (const [i, role] of allowedRoles.entries())
    await db.query("INSERT INTO public.user_roles(user_id,role) VALUES ($1,$2)", [
      uuid(i + 1),
      role,
    ]);
  for (const id of [20, 21, 23])
    await db.query("INSERT INTO public.user_roles(user_id,role) VALUES ($1,'corretor')", [
      uuid(id),
    ]);
  await db.query("INSERT INTO public.user_roles(user_id,role) VALUES ($1,'coordenador')", [
    uuid(24),
  ]);
  ...
  for (const id of [30, 31, 32, 33, 34])
    await db.query("INSERT INTO public.profiles(id,nome,ativo,email) VALUES ($1,$2,false,$3)", [
      uuid(id),
      `Histórico ${id}`,
      "SECRET_HISTORY",
    ]);
  ...
  for (const [id, owner, status] of [
    [200, 30, "ocorrencia_concluida"],
    [201, 1, "rascunho"],
    [202, 3, "rascunho"],
    [203, 30, "ocorrencia_concluida"],
  ]) {
    await db.query(
      "INSERT INTO public.sales(id,corretor_id,status,codigo_interno,imovel_id,modalidade,parceria_cpf_cnpj,parceria_pix,imovel_endereco) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)",
      [
        uuid(id),
        uuid(owner),
        status,
  ...
  for (const [id, sale, status, value, date] of [
    [300, 200, "concluida", 1000, "2026-08-01"],
    [301, 200, "concluida", null, null],
    [302, 201, "pendente", 2000, "2026-09-01"],
    [303, 202, "analise_financeiro", 3000, "2026-09-02"],
    [304, 203, "devolvida_gestor", 4000, null],
  ]) {
    await db.query(
      "INSERT INTO public.occurrences(id,sale_id,status,valor_comissao,data_assinatura,observacoes,financiamento_banco) VALUES ($1,$2,$3,$4,$5,$6,$7)",
      [uuid(id), uuid(sale), status, value, date, "SECRET_OBS", "SECRET_BANK"],
    );
  }
  await db.query(
    "INSERT INTO public.occurrence_commissions(id,occurrence_id,papel,nome,user_id,valor) VALUES ($1,$2,$3,$4,$5,$6)",
    [uuid(400), uuid(300), "corretor_vendedor", "Pessoa sintética 3", uuid(3), 500],
  );
  check(
    (await db.query("SELECT public.is_active_user($1) AS active", [uuid(21)])).rows[0].active,
    "effective old helper allows missing profile; RPC must not",
  );
  await assert.rejects(
    () => rpc(actors[0]),
    (e) => e.code === "42883",
  ...
  check(
    baseline["corretor/occurrences/select"].rows.every((r) => r.row.id !== uuid(300)),
    "cross-team completed occurrence is hidden directly",
  );
  ...
  equal(
    await state(),
    before,
    "migration preserves data, policies, table privileges/RLS and helpers",
  );
  const expected = {
    occs: ["id", "sale_id", "valor_comissao", "data_assinatura"],
    sales: ["id", "codigo_interno", "imovel_id", "corretor_id"],
    participants: ["occurrence_id", "user_id", "nome"],
    profiles: ["id", "nome"],
    teams: ["id", "nome", "parent_team_id", "lider_id"],
    members: ["membro_id", "team_id"],
    coLeaders: ["user_id", "team_id"],
  };
  let canonical;
  for (const actor of actors.slice(0, allowedRoles.length)) {
    ...
    equal(
      ...
      `${actor.name}: exact envelope`,
    );
    for (const [key, fields] of Object.entries(expected)) {
      check(Array.isArray(payload[key]), `${key} is array`);
      for (const row of payload[key])
        equal(Object.keys(row).sort(), fields.toSorted(), `${key} allowlist`);
      equal(
        new Set(payload[key].map((row) => row.id || JSON.stringify(row))).size,
        payload[key].length,
        `${key} unique`,
      );
    }
  ...
  equal(
    canonical.occs.map((o) => o.id),
    [uuid(300), uuid(301)],
    "only completed occurrences",
  );
  equal(
    canonical.sales.map((s) => s.id),
    [uuid(200)],
    "sales linked via occurrence status, unique despite multiple occurrences",
  );
  equal(
    canonical.participants.map((p) => p.occurrence_id),
    [uuid(300)],
    "completed occurrence participants",
  );
  ...
  equal(
    canonical.profiles.map((p) => p.id),
    [1, 2, 3, 8, 20, 23, 30, 31, 32, 33].map(uuid),
    "canonical roles plus inactive historical owner/leader/co-leader/member; unrelated excluded",
  );
  ...
  await assert.rejects(
    () =>
      asActor(actors[0], () =>
        db.query("SELECT public.relatorio_ocorrencias_concluidas($1::uuid)", [uuid(5)]),
      ),
    (e) => e.code === "42883",
  );
  checks++;
  const metadata = (
    await db.query(
      "SELECT prosecdef, provolatile, pronargs, proconfig, pg_get_function_result(oid) AS result FROM pg_proc WHERE oid='public.relatorio_ocorrencias_concluidas()'::regprocedure",
    )
  ).rows[0];
  equal(
    metadata,
    ...
    "security definer stable empty search path no args",
  );
  const afterMatrix = await directMatrix();
  equal(
    afterMatrix,
    baseline,
    "every direct SELECT/INSERT/UPDATE/DELETE unchanged across all actors/tables",
  );
  ...
  for (const t of [
    ...
    await db.exec(`DELETE FROM public.${ident(t)}`);
  ...
  equal(
    (await db.query("SELECT public.relatorio_ocorrencias_concluidas() AS payload")).rows[0].payload,
    Object.fromEntries(Object.keys(expected).map((k) => [k, []])),
    "all empty arrays, no nulls",
  );
  await db.exec("ROLLBACK");
    ...
    JSON.stringify(
      {
        ...
        directOperationsPerPhase: Object.keys(baseline).length,
        phases: ["before", "after", "rollback"],
        effectivePolicies: policies.length,
        effectiveHelpers: effectiveFunctions.length,
        versionedSupplementalHelpers,
        postgres: (await db.query("SELECT version() AS version")).rows[0].version,
        limits:
          "Synthetic fixture; not full constraints/triggers/grants, JWT/PostgREST, storage, or downstream policies. No remote access.",
      },
      null,
      2,
    ),
