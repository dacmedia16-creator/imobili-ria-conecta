-- PROBLEMA (regressão confirmada, achado #5 da auditoria seguinte): a correção anterior do ranking
-- de equipe (20260809120000) trocou o LEFT JOIN original por um JOIN interno em equipe_vendas_distintas,
-- o que excluiu inteiramente do ranking_equipe qualquer participante SEM equipe (ex.: gestor sem
-- vínculo em team_members) — antes ele aparecia agrupado sob team_id=NULL ("Sem equipe" no
-- front-end), depois da correção anterior ele simplesmente sumia da resposta da RPC.
--
-- Correção: LEFT JOIN em vez de JOIN (participante sem equipe vira team_id=NULL, preservado) — o
-- DISTINCT continua evitando duplicar a MESMA venda mesmo se 2+ participantes sem equipe estiverem
-- nela (colapsam pro mesmo (NULL, sale_id)). E o casamento de comissão em ranking_equipe_base precisa
-- de "IS NOT DISTINCT FROM" em vez de "=", já que NULL = NULL nunca é verdadeiro em SQL — com "="
-- puro o grupo "Sem equipe" apareceria com comissao=0 mesmo tendo dinheiro de verdade.
...
-- Venda distinta por equipe (LEFT JOIN — participante sem equipe vira team_id=NULL, preservado em vez
-- de sumir; DISTINCT ainda garante 1 linha por (equipe-ou-NULL, venda) mesmo com 2+ participantes na
-- mesma venda). dias/teve_devolucao são iguais pra qualquer participante da mesma venda (vêm de CTEs
-- por sale_id só), então o DISTINCT nunca perde informação diferente entre os participantes.
...
ranking_equipe_base AS (
  -- "IS NOT DISTINCT FROM" em vez de "=": NULL = NULL nunca é verdadeiro em SQL — com "=" puro o
  -- grupo "Sem equipe" apareceria zerado mesmo tendo comissão de verdade.
