-- Adiciona indicador de saúde do WhatsApp (item 6, frente 2) na visao_executiva_stats() — só
-- super_admin recebe esse campo no retorno (CASE WHEN has_any_role..., senão NULL), mesmo sendo
-- SECURITY INVOKER: admin comum continua vendo o resto da tela normalmente, só esse card fica de
-- fora. Últimos 30 dias, agregando activity_logs.payload (acao = 'whatsapp_notification_result').
