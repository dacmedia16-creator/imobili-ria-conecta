-- PROBLEMA (achado #8 da auditoria, confirmado com teste isolado): dentro de um trigger BEFORE
-- UPDATE, um SELECT contra a própria tabela enxerga a linha ANTIGA (ainda não gravada), nunca os
-- valores NEW pendentes da mesma operação — é assim que o Postgres funciona, não um bug de lógica.
-- validar_distribuicao_antes_aprovar_gestor() chamava calcular_distribuicao_venda(NEW.id), que por
-- baixo faz "select * from sales where id = ...", e portanto validava o estado ANTIGO da venda, não
-- o que está de fato sendo gravado na mesma instrução. Hoje isso não é explorável pela tela (o
-- Resumo salva os campos financeiros numa chamada separada, ANTES de mudar o status, então "antigo"
-- já é o valor final certo) — mas é uma fragilidade real pra qualquer chamada direta na API que
-- combine campos financeiros e status numa única atualização.
--
-- Correção: novo overload de calcular_distribuicao_venda que recebe a ROW diretamente (sales), sem
-- reconsultar a tabela. O trigger de venda passa NEW (que já contém os valores pendentes corretos)
-- em vez de NEW.id. O overload por uuid continua existindo (usado pelo front-end via RPC e pelo
-- trigger de Ocorrência, que corretamente consulta sales — ali NEW é a linha de OCCURRENCES sendo
-- alterada, não a de sales, então reconsultar sales pelo id está certo: sales não muda nessa transação).
...
-- Overload por id: só busca a row e delega pro overload acima — usado pelo front-end (RPC via
-- PostgREST, que não suporta chamar com um parâmetro do tipo composto "sales" diretamente) e pelo
-- trigger de Ocorrência (onde sales não é a tabela sendo alterada, então reconsultar está correto).
...
-- Trigger de venda: passa NEW (a linha inteira, com os valores pendentes desta MESMA atualização) em
-- vez de NEW.id — antes, calcular_distribuicao_venda(NEW.id) reconsultava a tabela e, por semântica
-- de trigger BEFORE UPDATE do Postgres, via a linha ANTIGA (a atualização ainda não foi gravada no
-- heap nesse ponto), não os valores sendo gravados nesta operação.
