-- Expõe os descontos de extras por origem individualmente (regra 2 do pedido de correção de bases
-- pra gestor/team leader/indicador/outros extras) — antes só existia o total combinado
-- (outros_extras); as variáveis por origem já existiam internamente, só não saíam no retorno.
-- Usado pelo front-end como base bruta pra converter percentual->valor de um extra sem cascata
-- (baseParaOrigem("imobiliaria") agora lê saldo_inicial_imobiliaria em vez do campo legado).
