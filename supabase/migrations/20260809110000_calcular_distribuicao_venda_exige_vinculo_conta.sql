-- Regra: só Parceria Externa pode ficar sem conta vinculada no sistema — captador, vendedor,
-- "outro captador/vendedor" e gestor/Team Leader são sempre gente interna da imobiliária, e por isso
-- precisam estar ligados a uma conta cadastrada (não só um nome digitado). Mesmo esquema das demais
-- inconsistências desta função: some no banner do Resumo/Ocorrência e bloqueia as 3 transições
-- críticas (envio ao jurídico, conclusão da Ocorrência, aceite financeiro) via os triggers já
-- existentes (20260809080000) — nenhum trigger novo precisou ser criado.
...
begin
  ...
  for v_extra_link in
    ...
    where sale_id = p_sale_id and nome is not null and user_id is null
