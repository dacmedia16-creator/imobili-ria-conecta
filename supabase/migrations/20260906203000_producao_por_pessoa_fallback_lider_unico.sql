-- Regra comercial autorizada: gestor e Team Leader também podem ocupar uma ponta de produção.
-- Em venda padrão, quando corretor_captador_id/corretor_vendedor_id estiver ausente, a RPC atribui
-- a ponta ao líder interno daquele lado somente se todas as fontes convergirem para uma única
-- pessoa. As fontes por lado são sales.lider_* e occurrence_commissions: papel lider_* ou papel
-- gestor/team_leader com lado explícito. Duplicatas da mesma pessoa não geram ambiguidade.
--
-- Deliberadamente não entram como candidatos:
-- - sales.corretor_id (criador da venda);
-- - sales.coordenador_id/team_leader_id sem lado (campos financeiros, não atribuição por ponta);
-- - parceria/occurrence_partners ou qualquer nome sem user_id/profile interno.
-- Com zero ou múltiplas pessoas internas candidatas, a ponta permanece sem vínculo. Lançamento
-- conserva integralmente o rateio existente por occurrence_commissions.papel = corretor_vendedor.
