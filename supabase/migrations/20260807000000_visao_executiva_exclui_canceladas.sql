-- Vendas canceladas/arquivadas continuavam contando em "Fechadas" e "Comissão" no ranking da
-- Visão Executiva, porque fechadas_30d/comissao_30d olhavam só o histórico de status (passou por
-- contrato_assinado alguma vez) e nunca o status atual da venda. Mesmo bug já corrigido em
-- comissoes-a-receber.tsx (commit b3e8689) — achado real: venda de teste do Denis, cancelada,
-- ainda aparecia em 1º lugar no ranking com R$ 43.800,00 de comissão.
...
-- Só conta como fechada se a venda ainda estiver "viva" hoje — cancelada/arquivada some do ranking
-- mesmo tendo passado por contrato_assinado no histórico.
tempo_fechamento AS (
