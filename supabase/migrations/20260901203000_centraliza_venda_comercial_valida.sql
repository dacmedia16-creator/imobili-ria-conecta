-- Regra comercial canônica de venda válida.
-- Venda padrão: conta enquanto estiver em contrato_assinado ou etapa posterior,
-- usando a assinatura mais recente. Se voltar a uma etapa anterior, deixa de contar.
-- Lançamento: como não passa por contrato_assinado, usa a entrada mais recente no financeiro.
-- Indicadores financeiros continuam com suas próprias funções e marcos de efetivação.

create or replace function public.vendas_comerciais_validas()
returns table (sale_id uuid, venda_em timestamptz)
language sql
stable
security invoker
...
revoke execute on function public.vendas_comerciais_validas() from public, anon;
grant execute on function public.vendas_comerciais_validas() to authenticated;
...
as $$
  with vendas_periodo as (
    ...
    select sale_id, count(*) as n from sale_status_history
    where para::text in ('devolvida_ajuste','ocorrencia_devolvida_gestor') group by sale_id
  ), participante_venda as (
    select oc.user_id, vp.sale_id, sum(oc.valor) as valor_na_venda,
    ...
    select user_id as corretor_id, count(*) as vendas_fechadas, avg(dias) as tempo_medio_dias,
      count(*) filter (where teve_devolucao) as vendas_com_devolucao, sum(valor_na_venda) as comissao
    from participante_venda group by user_id
  ), unidade as (
    ...
    select r.*, u.team_id from ranking_corretor_base r left join unidade u on u.corretor_id = r.corretor_id
  ), ranking_corretor as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'corretor_id', corretor_id, 'vendas_fechadas', vendas_fechadas,
    ...
    from participante_venda p left join unidade u on u.corretor_id = p.user_id
  ), ranking_equipe_base as (
    select ev.team_id, count(*) as vendas_fechadas,
    ...
    from equipe_vendas ev group by ev.team_id
  ), ranking_equipe as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'team_id', r.team_id, 'team_nome', t.nome, 'vendas_fechadas', r.vendas_fechadas,
      'comissao', r.comissao,
    ...
    from ranking_equipe_base r left join teams t on t.id = r.team_id
  ), captacoes as (
    select count(distinct id) as quantidade from sales
    where created_at >= _de::timestamptz and created_at < (_ate + 1)::timestamptz
...
as $$
  with vendas_periodo as (
    ...
    from public.vendas_comerciais_validas() v
    where v.venda_em >= _de::timestamptz and v.venda_em < (_ate + 1)::timestamptz
  ), participantes as (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'valor_comissao', p.valor_comissao, 'fechado_em', p.fechado_em, 'corretor_id', p.corretor_id
