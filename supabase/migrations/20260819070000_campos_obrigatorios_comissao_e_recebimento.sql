-- Fecha 3 gaps encontrados numa auditoria de "o que precisa estar preenchido pros números do
-- sistema baterem" (mesmo espírito da trava de vínculo de conta captador/vendedor já existente):
--
-- 1) Comissão bruta pode ficar em R$ 0 sem avisar (percentual_comissao E valor_total_comissao os
--    dois vazios) — as checagens de captador+vendedor/parceria vs. comissão bruta só rodam com
--    "v_comissao_bruta > 0", então tudo bate trivialmente (0 = 0) e a venda passa calculo_valido =
--    true sem nenhuma comissão informada. Vale pras duas modalidades (padrão e lançamento).
-- 2) Parceria externa marcada (parceria_tipo preenchido) sem informar quanto ela recebe
--    (parceria_percentual e parceria_valor os dois vazios/zero) — só existe no fluxo padrão
--    (Lançamento representa parceria via sale_commission_extras.sem_cadastro_confirmado, não usa
--    essas colunas de sales).
-- 3) Previsão de recebimento da Ocorrência (prev_recebimento{1,2,3}_valor) sem nenhuma validação em
--    lugar nenhum — uma Ocorrência pode ser concluída/travada pro financeiro sem nunca informar
--    quando/como a comissão vai ser recebida, e ela some de Comissões a Receber/Aging/Fluxo de Caixa
--    sem gerar alerta bloqueante (achado na ocorrência 630601113-11).
--
-- Igual às travas existentes: só passa a valer daqui pra frente (mesma garantia informal do NOT
-- VALID nas constraints já existentes) — nenhuma venda/ocorrência antiga é reavaliada por isto,
-- só as próximas vezes que alguém tentar uma das transições já protegidas.

-- ------------------------------------------------------------------------------------------------
-- 1) e 2): calcular_distribuicao_venda(sales) — mesma função única que atende padrão e lançamento
-- (branch por v_sale.modalidade). Corpo idêntico ao de 20260818000000, só com os 2 `if`s novos.
-- ------------------------------------------------------------------------------------------------
...
begin
  ...
  if v_sale.modalidade = 'lancamento' then
    ...
    -- NOVO (item 1): comissão bruta zerada com negociado já preenchido — antes passava calculo_valido
    -- = true trivialmente (0 pessoas + 0 parceria + 0 saldo = 0 comissão bruta).
  ...
  -- NOVO (item 1): mesma checagem do branch de Lançamento, agora pro fluxo padrão.
  if v_negociado > 0 and v_comissao_bruta <= 0 then
  ...
  -- NOVO (item 2): parceria marcada mas sem informar quanto ela recebe — antes ficava valendo R$ 0
  -- silenciosamente e seguia em frente.
...
-- ------------------------------------------------------------------------------------------------
-- 3) Previsão de recebimento da Ocorrência — nova função, mesmo formato de retorno
-- ({inconsistencias, calculo_valido}) que calcular_distribuicao_venda, pra poder ser mesclada com
-- ela no trigger abaixo. Considera prêmio (premio_valor) igual a financeiro-dashboard-query.ts já
-- faz no front-end (comissão esperada = valor_comissao + premio_valor).
-- ------------------------------------------------------------------------------------------------
...
begin
  ...
  v_esperado := coalesce(p_occ.valor_comissao, 0) + coalesce(p_occ.premio_valor, 0);

  if v_esperado > 0 and v_soma <= 0 then
    v_inconsistencias := v_inconsistencias || jsonb_build_array(
      'Previsão de recebimento não informada — a comissão da Ocorrência é R$ ' || round(v_esperado, 2) || ' mas nenhuma parcela prevista foi cadastrada.'
    );
  elsif v_esperado > 0 and abs(v_soma - v_esperado) > 0.01 then
    v_inconsistencias := v_inconsistencias || jsonb_build_array(
      'Previsão de recebimento (R$ ' || round(v_soma, 2) || ') não bate com a comissão da Ocorrência (R$ ' || round(v_esperado, 2) || ') — confira os valores das parcelas previstas.'
    );
  end if;
...
-- ------------------------------------------------------------------------------------------------
-- Estende o trigger de Ocorrência já existente (mesmos 2 gatilhos: concluida / aceita_financeiro)
-- pra também rodar validar_previsao_recebimento e mesclar as inconsistências das duas checagens
-- numa única mensagem de erro — sem trigger novo, só mais uma fonte de inconsistências no mesmo ponto.
-- ------------------------------------------------------------------------------------------------
...
begin
  ...
  v_prev := public.validar_previsao_recebimento(NEW);
  v_inconsistencias := coalesce(v_dist->'inconsistencias', '[]'::jsonb) || coalesce(v_prev->'inconsistencias', '[]'::jsonb);

  if jsonb_array_length(v_inconsistencias) > 0 then
    raise exception 'Não é possível % desta venda: %', v_acao, (
      select string_agg(x, '; ') from jsonb_array_elements_text(v_inconsistencias) x
    ) using errcode = '23514';
  end if;
  return NEW;
end;
