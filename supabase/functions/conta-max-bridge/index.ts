...
async function verify(ticket: string, secret: string) {
  try {
    ...
    if (!(await crypto.subtle.verify("HMAC", key, bytes(signature), encoder.encode(encoded))))
      return null;
    const payload = JSON.parse(new TextDecoder().decode(bytes(encoded)));
    const now = Math.floor(Date.now() / 1000);
    if (payload.iss !== "conta-max" || payload.aud !== "adm-max" || payload.app !== "adm-max")
      return null;
    if (
      !payload.sub ||
      !payload.jti ||
    ...
    return payload.email.includes("@") ? payload : null;
  } catch {
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  const secret = Deno.env.get("ADM_MAX_BRIDGE_SECRET") ?? "";
  const url = Deno.env.get("SUPABASE_URL") ?? "";
  const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
  const anonKey = Deno.env.get("SUPABASE_ANON_KEY") ?? "";
  if (secret.length < 32 || !url || !serviceKey || !anonKey)
  ...
  if (verifyError || !verified.session)
    return response({ error: "session_generation_failed" }, 500);
  return response({
